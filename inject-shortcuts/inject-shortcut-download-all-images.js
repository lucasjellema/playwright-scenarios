...
const shortCutExecutor = async (source) => {
...
(async () => {
  ...
  await context.exposeBinding('imageDownloadFunction', shortCutExecutor)
  ...
  const shortCutJS  = `function handleShortCutKey(e) { if ('KeyB'==e.code && e.ctrlKey) // ctrl + b
                             window.imageDownloadFunction() ;  // invoke the Node function that was exposed to the browser context
